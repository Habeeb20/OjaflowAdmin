...
import { Users, Search, ChevronLeft, ChevronRight, AlertCircle } from 'lucide-react';
...
const AdminUsersPage = () => {
  ...
  const fetchUsers = async (page = 1, search = '') => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/admin/list/users`, {
        headers: {
                  ...
                  {users.length === 0 ? (
                    ...
                    users.map((user) => (
                      <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{user.state}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{user.industry}</td>
                      </tr>
                    ))
                  )}
                </tbody>
