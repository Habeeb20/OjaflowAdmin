...
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { Search, ChevronLeft, ChevronRight, AlertCircle, Building, Plus, Edit, Trash2, X } from 'lucide-react';
import { useNotification } from '../../utils/NotificationSystem.jsx';
import Navbar from '../../components/Navbar.jsx';
import Loading from '../../utils/Loading.jsx';
...
const WarehousesList = () => {
  ...
  const [showModal, setShowModal] = useState(false); // For showing details modal
  const [selectedWarehouse, setSelectedWarehouse] = useState(null); // For modal content
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    address: '',
  ...
  const handleStoreWarehouse = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/warehouses/store`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      addNotification('Warehouse created successfully!', 'success');
    } catch (err) {
      addNotification('Failed to create warehouse: ' + err.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/warehouses/update/${editingWarehouse.slug}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      const data = await response.json();
      setWarehouses(warehouses.map(w => w.slug === editingWarehouse.slug ? data : w));
      setEditMode(false);
      setEditingWarehouse(null);
      addNotification('Warehouse updated successfully!', 'success');
    } catch (err) {
      addNotification('Failed to update warehouse: ' + err.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (slug) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/warehouses/delete/${slug}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      setWarehouses(warehouses.filter(w => w.slug !== slug));
      addNotification('Warehouse deleted successfully!', 'success');
    } catch (err) {
      addNotification('Failed to delete warehouse: ' + err.message, 'error');
    } finally {
      setLoading(false);
    }
  };

  // Show warehouse details in modal
  ...
  const handleEdit = (warehouse) => {
    ...
    setEditMode(true);
    setShowForm(false); // Ensure add form is hidden when editing
  };
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {(showForm || editMode) && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 mb-6">
              ...
              <form onSubmit={editMode ? handleUpdate : handleStoreWarehouse} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Name</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder="5000"
                      className="flex-1 p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-400 bg-white dark:bg-gray-700"
                      required
                    />
                    <input
                      type="text"
                      ...
                      onChange={handleInputChange}
                      placeholder="cubic meters"
                      className="w-32 p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-400 bg-white dark:bg-gray-700"
                      required
                    />
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
                    {Object.keys(formData.operating_hours).map((day) => (
                      <input
                        ...
                        className="p-2 border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-primary-400 bg-white dark:bg-gray-700 text-xs"
                      />
                    ))}
                  </div>
                </div>
                <div className="md:col-span-2 space-y-2">
                  <label className="flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm text-gray-900 dark:text-gray-100">Is Active</span>
                  </label>
                  <label className="flex items-center">
                    <input
                      type="checkbox"
                      ...
                      className="h-4 w-4 text-primary-600 focus:ring-primary-400 border-gray-300 rounded"
                    />
                    <span className="ml-2 text-sm text-gray-900 dark:text-gray-100">Is Public</span>
                  </label>
                </div>
                <div className="md:col-span-2 flex justify-end space-x-2">
                  <button
                  ...
                  >
                    {loading ? 'Saving...' : (editMode ? 'Update Warehouse' : 'Save Warehouse')}
                  </button>
                </div>
              </form>
            </div>
                    ...
                    warehouses.map((warehouse) => (
                      <tr key={warehouse.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          ...
                          <button
                            onClick={() => handleDelete(warehouse.slug)}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Warehouse Details</h3>
                  <button
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-gray-100">{selectedWarehouse.name}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
                    <p className="text-gray-900 dark:text-gray-100">{selectedWarehouse.description}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-gray-100">
                      {selectedWarehouse.city}, {selectedWarehouse.state}, {selectedWarehouse.country} {selectedWarehouse.postal_code}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Contact</label>
                    <p className="text-gray-900 dark:text-gray-100">
                      {selectedWarehouse.contact_number} | {selectedWarehouse.email_contact}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Manager</label>
                    <p className="text-gray-900 dark:text-gray-100">{selectedWarehouse.manager_name}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Type</label>
                    <p className="text-gray-900 dark:text-gray-100">{selectedWarehouse.type}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Capacity</label>
                    <p className="text-gray-900 dark:text-gray-100">{selectedWarehouse.capacity} {selectedWarehouse.capacity_unit}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Status</label>
                    <p className="text-gray-900 dark:text-gray-100">
                      {selectedWarehouse.is_active ? 'Active' : 'Inactive'} | {selectedWarehouse.is_public ? 'Public' : 'Private'}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Notes</label>
                    <p className="text-gray-900 dark:text-gray-100">{selectedWarehouse.notes || 'N/A'}</p>
                  </div>
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {Object.entries(selectedWarehouse.operating_hours).map(([day, hours]) => (
                        <p key={day} className="text-gray-900 dark:text-gray-100">
